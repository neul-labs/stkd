...
use common::{run_git, run_stack, run_stack_success, TestContext};

#[test]
fn test_init_creates_stack_directory() {
    let ctx = TestContext::new();

    run_stack_success(&ctx.path, &["init"]);

    // Verify .git/stack directory exists with config and state files
    assert!(ctx.path.join(".git/stack").exists());
    assert!(ctx.path.join(".git/stack/config.json").exists());
    assert!(ctx.path.join(".git/stack/state.json").exists());
}
...
fn test_init_warns_if_already_initialized() {
    ...
    let output1 = run_stack(&ctx.path, &["init"]);
    assert!(output1.status.success());

    // Second init should succeed but show a warning
    let output2 = run_stack(&ctx.path, &["init"]);
...
fn test_create_branch() {
    let ctx = TestContext::with_stack();

    run_stack_success(&ctx.path, &["create", "feature/test"]);
...
fn test_create_branch_with_parent() {
    ...
    run_stack_success(&ctx.path, &["create", "feature/parent"]);
    ctx.commit_file("parent.txt", "parent content", "Add parent file");

    // Create child branch on top of parent
    run_stack_success(&ctx.path, &["create", "feature/child"]);
...
fn test_create_from_trunk() {
    ...
    // Create first branch
    run_stack_success(&ctx.path, &["create", "feature/first"]);
    ctx.commit_file("first.txt", "first", "Add first");

    // Create second branch from trunk (not from first)
    run_stack_success(&ctx.path, &["create", "--from-trunk", "feature/second"]);
...
fn test_navigation_up_down() {
    ...
    run_stack_success(&ctx.path, &["create", "feature/a"]);
    ctx.commit_file("a.txt", "a", "Add a");

    run_stack_success(&ctx.path, &["create", "feature/b"]);
    ctx.commit_file("b.txt", "b", "Add b");

    run_stack_success(&ctx.path, &["create", "feature/c"]);
    ctx.commit_file("c.txt", "c", "Add c");

    // We're at feature/c, go down
    run_stack_success(&ctx.path, &["down"]);
...
fn test_navigation_top_bottom() {
    ...
    run_stack_success(&ctx.path, &["bottom"]);
    assert_eq!(ctx.current_branch(), "feature/a");

    // Go to top (feature/c)
    run_stack_success(&ctx.path, &["top"]);
...
fn test_log_shows_stack() {
    ...
    let output = run_stack_success(&ctx.path, &["log"]);
...
fn test_status_command() {
    ...
    let output = run_stack_success(&ctx.path, &["status"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_delete_branch() {
    ...
    // Create a branch
    run_stack_success(&ctx.path, &["create", "feature/to-delete"]);
    ...
    // Delete the branch
    run_stack_success(&ctx.path, &["delete", "feature/to-delete", "--force"]);
...
fn test_rename_branch() {
    ...
    run_stack_success(&ctx.path, &["create", "feature/old-name"]);
    assert!(ctx.branch_exists("feature/old-name"));

    run_stack_success(&ctx.path, &["rename", "feature/new-name"]);
...
fn test_checkout_by_name() {
    ...
    // Go to a
    run_stack_success(&ctx.path, &["checkout", "feature/a"]);
    assert_eq!(ctx.current_branch(), "feature/a");

    // Go to b
    run_stack_success(&ctx.path, &["checkout", "feature/b"]);
    assert_eq!(ctx.current_branch(), "feature/b");
}
...
fn test_fails_without_init() {
    ...
    let output = run_stack(&ctx.path, &["create", "feature/test"]);
...
fn test_modify_amends_commit() {
    ...
    run_stack_success(&ctx.path, &["modify"]);
...
fn test_version_command() {
    ...
    let output = run_stack_success(&ctx.path, &["--version"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_help_command() {
    ...
    let output = run_stack_success(&ctx.path, &["--help"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
