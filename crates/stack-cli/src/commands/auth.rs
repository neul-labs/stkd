...
use anyhow::Result;
use clap::Args;
use stack_github::{auth as github_auth, DeviceFlow, GitHubClient};
use stack_gitlab::auth as gitlab_auth;
...
pub async fn execute(args: AuthArgs) -> Result<()> {
    ...
    let provider = if args.github {
        ...
        match stack_core::Repository::open(".") {
...
async fn show_gitlab_status(host: &str) -> Result<()> {
    if let Some(token) = gitlab_auth::load_credentials(host)? {
        ...
        let provider = if host == "gitlab.com" {
            stack_gitlab::GitLabProvider::new(&token.token)?
        } else {
            stack_gitlab::GitLabProvider::with_host(&token.token, host)?
        };

        use stack_provider_api::UserProvider;
...
async fn gitlab_token_auth(token: String, host: &str) -> Result<()> {
    ...
    let provider = if host == "gitlab.com" {
        stack_gitlab::GitLabProvider::new(&token)?
    } else {
        stack_gitlab::GitLabProvider::with_host(&token, host)?
    };
