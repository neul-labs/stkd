...
use stack_provider_api::{Provider, RepoId, RepositoryProvider};
...
impl ProviderContext {
    ...
    pub async fn from_repo(repo: &Repository) -> Result<Self> {
        ...
        let (provider, repo_id): (Box<dyn Provider>, RepoId) = match provider_type {
            ProviderType::GitHub => {
                let auth_token = stack_github::auth::load_credentials()?
                    .ok_or_else(|| anyhow::anyhow!("Not authenticated with GitHub. Run 'gt auth --github' first."))?;

                let provider = stack_github::GitHubProvider::with_auth(auth_token.to_auth())?;
            ...
            ProviderType::GitLab => {
                let auth_token = stack_gitlab::auth::load_credentials(&host)?
                    .ok_or_else(|| anyhow::anyhow!("Not authenticated with GitLab. Run 'gt auth --gitlab' first."))?;

                let provider = if host == "gitlab.com" {
                    stack_gitlab::GitLabProvider::new(auth_token.token)?
                } else {
                    stack_gitlab::GitLabProvider::with_host(auth_token.token, &host)?
                };
...
pub fn has_any_credentials() -> bool {
    stack_github::auth::load_credentials().ok().flatten().is_some()
        || stack_gitlab::auth::load_credentials("gitlab.com").ok().flatten().is_some()
}
