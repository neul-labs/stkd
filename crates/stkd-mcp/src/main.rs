...
impl ServerHandler for StkdMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "Stack (stkd) MCP server: manage stacked branches and merge requests.",
        )
    }
}
...
impl StkdMcpServer {
    ...
    fn gt_log(
        &self,
        Parameters(_req): Parameters<LogRequest>,
    ) -> Result<CallToolResult, rmcp::ErrorData> {
        let (repo, _lock) = open_repo_locked()?;
                ...
                if req.fetch.unwrap_or(false) {
                    let result = tokio::task::block_in_place(|| {
                        tokio::runtime::Handle::current().block_on(async {
                            let ctx = stkd_engine::ProviderContext::from_repo(&repo)
                                .await
                                .map_err(to_mcp_error)?;
                            let mr = ctx
                                .provider()
                                .get_mr(
                                    &ctx.repo_id,
                                    stkd_provider_api::MergeRequestId::from(mr_number),
                                )
                                .await
                                .map_err(to_mcp_error)?;
    ...
    fn gt_submit(
        ...
        let result = tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                let result = stkd_engine::submit(&repo, opts, ctx.provider(), &ctx.repo_id)
                    .await
                    .map_err(to_mcp_error)?;
    ...
    #[tool(
        description = "Sync with remote: fetch, update trunk, check MR status, delete merged branches, and restack"
    )]
    fn gt_sync(
        &self,
        ...
        let result = tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                let result = stkd_engine::sync(&repo, opts, provider_ref, repo_id_ref)
                    .await
                    .map_err(to_mcp_error)?;
                Ok::<_, rmcp::ErrorData>(result)
            })
    ...
    fn gt_land(
        &self,
        Parameters(req): Parameters<LandRequest>,
    ) -> Result<CallToolResult, rmcp::ErrorData> {
        let (repo, _lock) = open_repo_locked()?;
        ...
        let result = tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                let result = stkd_engine::land(&repo, opts, ctx.provider(), &ctx.repo_id)
                    .await
                    .map_err(to_mcp_error)?;
                Ok::<_, rmcp::ErrorData>(result)
            })
    ...
    fn gt_restack(
        &self,
        Parameters(req): Parameters<RestackRequest>,
    ) -> Result<CallToolResult, rmcp::ErrorData> {
        let (repo, _lock) = open_repo_locked()?;
    ...
    fn gt_track(
        &self,
        Parameters(req): Parameters<TrackRequest>,
    ) -> Result<CallToolResult, rmcp::ErrorData> {
        let (repo, _lock) = open_repo_locked()?;
    ...
    fn gt_delete(
        ...
        let (repo, _lock) = open_repo_locked()?;
        repo.delete_branch(&req.branch, req.force.unwrap_or(false))
            .map_err(to_mcp_error)?;
