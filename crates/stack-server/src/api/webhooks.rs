...
async fn handle_github_pr_event(
    ...
    let mr_state = if pr.merged.unwrap_or(false) {
        stack_db::MergeRequestState::Merged
    } else if pr.draft.unwrap_or(false) {
        stack_db::MergeRequestState::Draft
    } else if pr.state == "closed" {
        stack_db::MergeRequestState::Closed
    } else {
        stack_db::MergeRequestState::Open
    };
    ...
    let branch_status = match mr_state {
        stack_db::MergeRequestState::Merged => stack_db::BranchStatus::Merged,
        stack_db::MergeRequestState::Closed => stack_db::BranchStatus::Closed,
        _ => stack_db::BranchStatus::Active,
    };
...
async fn handle_gitlab_mr_event(
    ...
    let mr_state = match mr.state.as_str() {
        "merged" => stack_db::MergeRequestState::Merged,
        "closed" => stack_db::MergeRequestState::Closed,
        _ => stack_db::MergeRequestState::Open,
    };
