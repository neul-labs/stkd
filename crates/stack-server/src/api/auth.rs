    ...
    let url = match provider.as_str() {
        ...
        "gitlab" => {
            let config = state
                ...
                .gitlab
                .as_ref()
                .ok_or_else(|| ApiError::BadRequest("GitLab OAuth not configured".to_string()))?;
...
async fn oauth_callback(
    ...
    let user_info = match provider.as_str() {
        "github" => {
            let config = state
                ...
                .ok_or_else(|| ApiError::BadRequest("GitHub OAuth not configured".to_string()))?;
            let oauth = GitHubOAuth::new(config.clone(), redirect_uri);
            let tokens = oauth.exchange_code(&callback.code).await
                .map_err(|e| ApiError::Internal(e.to_string()))?;
            oauth.get_user_info(&tokens.access_token).await
                .map_err(|e| ApiError::Internal(e.to_string()))?
        }
        "gitlab" => {
            let config = state
    ...
    let session = stack_db::Session::new(user.id, token_hash, state.config().jwt_expiry_days);
