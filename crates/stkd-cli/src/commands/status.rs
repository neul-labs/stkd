...
async fn show_mr_details(repo: &Repository, mr_number: u64) -> Result<()> {
    ...
    let state_str = match mr.state {
        stkd_provider_api::MergeRequestState::Open => "Open".green(),
        stkd_provider_api::MergeRequestState::Closed => "Closed".red(),
        stkd_provider_api::MergeRequestState::Merged => "Merged".purple(),
        stkd_provider_api::MergeRequestState::Draft => "Draft".yellow(),
    };
