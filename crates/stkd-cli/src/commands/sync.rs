...
pub async fn execute(args: SyncArgs, json: bool) -> Result<()> {
    ...
    if let Some(interval) = args.watch {
        ...
        loop {
            ...
            if !json {
                output::info("");
                output::info(&format!("Next sync in {} seconds...", interval_secs));
            }
    ...
    if args.dry_run {
        if json {
            println!(
                "{}",
                serde_json::to_string_pretty(
                    &serde_json::json!({"dry_run": true, "actions": ["fetch", "update_trunk", "check_mr_status", "delete_merged", "restack"]})
                )?
            );
            return Ok(());
        }
        ...
        output::info(&format!(
            "  {} Check MR status for tracked branches",
            output::ARROW
        ));

        if !args.no_delete {
            output::info(&format!(
                "  {} Delete branches with merged MRs",
                output::ARROW
            ));
        }
        ...
        if !args.no_restack {
            ...
            if !needs_restack.is_empty() || args.force {
                output::info(&format!(
                    "  {} Restack branches that need updating:",
                    output::ARROW
                ));
